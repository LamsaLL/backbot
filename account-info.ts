...
class AccountInfoDisplay {
  async getCompleteAccountInfo() {
    ...
    try {
      ...
      console.log(`🔒 Mode: ${isSimulation ? "SIMULATION" : "⚠️  LIVE TRADING"}`);
      console.log("");
      ...
      if (account) {
        ...
        console.log(`✅ Futures Maker Fee: ${account.futuresMakerFee || 'N/A'}`);
      ...
      if (collateral) {
        console.log(`💰 Net Equity: $${parseFloat(collateral.netEquity).toFixed(2)}`);
        console.log(`💵 Available: $${parseFloat(collateral.available).toFixed(2)}`);
        console.log(`🔒 Locked: $${parseFloat(collateral.locked).toFixed(2)}`);
        console.log(`📊 Initial Margin: $${parseFloat(collateral.initialMargin).toFixed(2)}`);
        console.log(`⚠️  Maintenance Margin: $${parseFloat(collateral.maintenanceMargin).toFixed(2)}`);
        
        const marginRatio = (parseFloat(collateral.maintenanceMargin) / parseFloat(collateral.netEquity)) * 100;
      ...
      if (positions && positions.length > 0) {
        ...
        positions.forEach(pos => {
          ...
          console.log(`   Unrealized PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`);
          console.log(`   Notional: $${parseFloat(pos.notionalValue || '0').toFixed(2)}`);
          console.log("");
        });
        
        console.log(`💰 Total Unrealized PnL: ${totalPnL >= 0 ? '+' : ''}$${totalPnL.toFixed(2)}`);
      } else {
      ...
      if (botAccount) {
        console.log(`💰 Available Capital: $${botAccount.capitalAvailable.toFixed(2)}`);
        console.log(`📊 Min Volume per Trade: $${botAccount.minVolumeDollar.toFixed(2)}`);
      ...
      console.log("-".repeat(40));
      console.log(`🔒 Simulation Mode: ${isSimulation ? '✅ ENABLED' : '❌ DISABLED'}`);
