...
class RiskManager {
  ...
  constructor(customLimits?: Partial<RiskLimits>) {
    this.limits = {
      maxRiskPerTrade: parseFloat(process.env.MAX_RISK_PER_TRADE || "0.02"), // Use env variable
      ...
      minVolumeUSD: parseFloat(process.env.MIN_VOLUME_USD || "50"), // LOWER minimum for small capital
      maxVolumeUSD: parseFloat(process.env.MAX_VOLUME_USD || "200"), // Use env variable
      stopLossRequired: process.env.REQUIRE_STOP_LOSS !== "false", // Default true
    ...
    console.log(`🛡️ RiskManager initialized:
      ...
      - Volume Range: $${this.limits.minVolumeUSD} - $${
  ...
  async validateNewPosition(
    ...
    console.log(
      `🔍 Validating position: ${symbol} - Volume: $${volume.toFixed(2)}`
    );
    ...
    const positionLimitCheck = this.checkPositionLimits(
      symbol,
      existingPositions
    );
    if (!positionLimitCheck.isValid) {
      console.log(
        `❌ Position limit check failed: ${positionLimitCheck.reason}`
      );
      return positionLimitCheck;
    }

    // 5. Check total exposure
    const exposureCheck = this.checkTotalExposure(
      volume,
      existingPositions,
      capitalAvailable
    );
    ...
    const riskAmount = stopLoss
      ? Math.abs(entryPrice - stopLoss) * (volume / entryPrice)
      : volume * 0.05; // Assume 5% risk if no stop
    const riskPercentage = riskAmount / capitalAvailable;

    console.log(
      `✅ Position validation passed - Risk: ${(riskPercentage * 100).toFixed(
        2
      )}%`
    );
  ...
  validatePositionSize(
    ...
    if (volume > maxRiskAmount) {
      ...
      return {
        isValid: false,
        reason: `Position exceeds ${(this.limits.maxRiskPerTrade * 100).toFixed(
          1
        )}% risk limit`,
  ...
  async checkDailyLossLimit(
    ...
    if (todayPnL && todayPnL.totalPnL < 0) {
      const dailyLossPercentage =
        Math.abs(todayPnL.totalPnL) / capitalAvailable;

      if (dailyLossPercentage >= this.limits.maxDailyLoss) {
        return {
          isValid: false,
          reason: `Daily loss limit reached: ${(
            dailyLossPercentage * 100
          ).toFixed(2)}%`,
        };
      }
    }
  ...
  checkPositionLimits(
    ...
    const positionsInMarket = existingPositions.filter(
  ...
  checkTotalExposure(
    ...
    const currentExposure = existingPositions.reduce((total, pos) => {
      const positionValue =
        Math.abs(parseFloat(pos.netQuantity)) *
    ...
    if (exposurePercentage > this.limits.maxTotalExposure) {
      const maxAllowedVolume =
        capitalAvailable * this.limits.maxTotalExposure - currentExposure;

      return {
        isValid: false,
        reason: `Total exposure would exceed ${(
          this.limits.maxTotalExposure * 100
        ).toFixed(1)}%`,
  ...
  calculateSafePositionSize(
    ...
    // Apply volume limits
    return Math.min(maxVolume, this.limits.maxVolumeUSD);
  }
  ...
  updateDailyPnL(pnl: number): void {
    ...
    console.log(
      `📊 Daily P&L updated: ${pnl >= 0 ? "+" : ""}$${pnl.toFixed(2)}`
    );
  }
  ...
  getRiskMetrics(
    ...
    return {
      ...
      dailyPnLPercentage: todayPnL
        ? (todayPnL.totalPnL / capitalAvailable) * 100
        : 0,
      remainingRiskCapacity: Math.max(
        0,
        capitalAvailable * this.limits.maxRiskPerTrade
      ),
      canOpenNewPosition:
        existingPositions.length < this.limits.maxOpenPositions,
    };
  }
   ...
   */
  shouldHaltTrading(capitalAvailable: number): {
    halt: boolean;
    reason?: string;
  } {
    const today = new Date().toISOString().split("T")[0];
